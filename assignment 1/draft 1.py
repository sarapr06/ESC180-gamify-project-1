...
#^^^^but NOT if they rested before
#resting and carrying textbooks =-2hedons/min if user is TIRED (finished running/carrying textbooks less than 2h before current activity) and not using a star. e.g. user tired if run for 2 mins then run again straight away
#if not tired, running = 2hedon/min for first 10 mins of running, -2 hedons/min for every minute after first 10
#if not tired, carrying textbooks = 1hedon/min for first 20 mins, -1hedon/min for every minute after first 20
...
def perform_activity(activity, duration): #duration is positive int. if activity is NOT running, textbooks, or resting, running the function should have no effect
    ...
    global tired_duration #if in the range of 2h, then tired. if not, then not tired
    if activity == "resting":
        hedons = hedons + 0
